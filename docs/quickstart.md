...
print(result.outputs["total"].is_verified())
print(result.notes)
PY
```
...
```text
{'total': Verified(...)} plus a verified value of `15`
```
...
built-in generator for formal `TATVA [SumNumbers]` contracts. Unknown goals
still compile into safe scaffolds that preserve constraints and verification metadata.

## What to try next

1. Inspect `examples/diagnose_disease.sutra` to see SAMVAD agents, SMRITI
   memory, VIKASA gates, and cognitive types.
2. Copy `examples/sum_numbers.msutra` or `examples/sutra_sum.sutra` and change the goal, inputs, constraints,
   and outputs.
3. Run `inspect` to verify the semantic graph.
4. Run `compile` to produce Python.
5. Add a domain generator in `manas_sutra/codegen.py` when you want the compiler
