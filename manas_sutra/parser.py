...
SUTRA_BLOCK_ALIASES = {
    ...
    "SANDESHA": "agents",
    "NIRNAYA": "agents",
    "VIKASA": "self_evolution",
    "KSHETRA_ANUMATI": "self_evolution",
    "NITYA_PRATIBANDH": "self_evolution",
    "PRAMAN_AVASHYAK": "self_evolution",
    "PRAKRIYA": "self_evolution",
    "SWAYAM_WAPAS": "self_evolution",
    "ITIHAAS": "self_evolution",
    "PHALA": "outputs",
}
...
class ManasProgram:
    ...
    self_evolution: tuple[str, ...] = field(default_factory=tuple)
    cognitive_types: tuple[str, ...] = field(default_factory=tuple)
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "self_evolution": list(self.self_evolution),
            "cognitive_types": list(self.cognitive_types),
            "raw_sections": {
...
def _parse_sutra_contract(source: str) -> ManasProgram:
    ...
    for line_number, raw_line in enumerate(source.splitlines(), start=1):
        ...
        if block_match:
            ...
            section = SUTRA_BLOCK_ALIASES.get(block_name)
            if section is not None:
                current_section = section
                section_depth = 1
                continue
            if current_section is not None:
                sections[current_section].append(_normalize_item(line))
                section_depth += 1
                continue
        ...
        if line == "}":
            if current_section is not None and section_depth > 1:
...
def _build_program(
    *, goal: str, sections: dict[str, list[str]], dialect: str, contract: str
    ...
    return ManasProgram(
        ...
        self_evolution=_unique_preserving_order(raw_sections.get("self_evolution", ())),
        cognitive_types=_extract_cognitive_types(raw_sections),
        dialect=dialect,
        contract=contract,
...
def _extract_cognitive_types(raw_sections: Mapping[str, tuple[str, ...]]) -> tuple[str, ...]:
    type_names: list[str] = []
    for items in raw_sections.values():
        for item in items:
            type_names.extend(re.findall(r"\b[A-Z][A-Za-z0-9_]*\s*<", item))
    return _unique_preserving_order(name.rstrip("<").strip() for name in type_names)
